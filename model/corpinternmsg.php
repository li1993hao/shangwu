...
//        }
    ...
    public function getappzpcontent($userId=0,$cim_id,$type){
        if($type==1){
            ...
            $sql="select w.ww_title title,w.ww_time fb_date ,w.ww_read read_num,w.ww_content content,p.pic_link from westwork w left join picture p on w.pic_id=p.pic_id where ww_id=$cim_id";
            $sql1="update westwork set ww_read=ww_read+1 where ww_id=$cim_id";
            $this->update($sql1);
        }elseif($type==7){
            $sql="select si.si_title title,si.si_time fb_date,si.si_scan read_num,si.si_content content,f.file_link,si.file_name from sourceinformation si left join file f on si.file_id=f.	file_id where si.si_id=$cim_id";
            //var_dump($sql);
    ...
    public function gethotnews2($type_num,$num,$start_time,$end_time){
        if($type_num==1){
            ...
            $sql="SELECT jm_id info_id,jm_name info_name,jm_addr info_content,jm_read info_read,jm_good info_good,jm_opentime info_date FROM jobfairmsg WHERE  `jm_date` >  '".$end_time."' AND  `jm_date` <  '".$start_time."'  order by jm_good desc limit $num,10";
        }
        return $this->fetchAll($sql);
    }
