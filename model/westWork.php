...
class westwork extends Model{
     ...
     */
    public function getlistinfo($type,$num){
        $sql = "SELECT `westwork`.*,`picture`.* FROM `westwork` LEFT JOIN `picture` ON `westwork`.`pic_id` = `picture`.`pic_id`  WHERE `westwork`.`wc_id` = '".$type."' ORDER BY  `westwork`.`ww_isup` DESC , `westwork`.`ww_time` DESC Limit $num,10 ";
        return $this->fetchAll($sql);
    }
