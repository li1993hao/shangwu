...
class jobinfo extends Model{
    ...
    {
        $sql = "SELECT `jobinfo`.*,`picture`.* FROM `jobinfo` LEFT JOIN `picture` ON `jobinfo`.`pic_id` = `picture`.`pic_id`
				WHERE `jobinfo`.`it_id` = '".$type."' ORDER BY   `jobinfo`.`ji_isup` DESC , `jobinfo`.`ji_date` DESC Limit ".$num.",10";
    ...
    public function do_cy_good($ji_id){
        $sql="update jobinfo set ji_good=ji_good+1 where ji_id=$ji_id";
        return $this->update($sql);
    }
