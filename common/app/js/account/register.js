$(function() {
    ...
    $(".e-mail").blur(function() {
        var val = $(this).val();
        var elm = $(".form-register-item-warn");
        elm.html("正在验证...");
        if (!isEmail(val)) {
            elm.html("email格式不正确！");
            email_flag = false;
    ...
    $(".re-pw").blur(function() {
        var val = $(this).val();
        if (val == "" || val != $(".pw").val()) {
            $(".form-register-item-warn").html("两次密码不一致！");
        }
    });
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if (msg.json.state == 1) {
                    ...
                    $(".form-register-item-warn").html(msg.json.msg);
                }

            },
            error: function(o) {
                $(".form-register-item-warn").html("网络连接出错！");
            }
        });
    });
    ...
    $("#form-register").submit(function() {
        ...
        if (!email_flag) {
            ...
            $(".form-register-item-warn").html("邮箱填写错误!");
            $(".e-mail").focus();
            return false;
        }
    ...
    function isEmpty(obj) {
        var val = obj.val();
        if (val == "") {
           $(".form-register-item-warn").html(obj.attr("error_info"));
            obj.focus();
            return true;
        }
        $(".form-register-item-warn").html("<span style='color:green;'>通过<span>");
        return false;
    }
});