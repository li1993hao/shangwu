...
//	}
    ...
    /** 获取基层就业信息列表 */
    public function  Jcinfo(){
        $type = $this->getRequest ()->get ( "type" );
        $num = $this->getRequest ()->get ( "num" );
        //var_dump($num);
        $westwork=new westWork();
        /** 获取工作动态，相关政策，典型人物信息列表 */
        $info_arr=$westwork->getlistinfo($type,$num);
        //var_dump($info_arr);
        if($info_arr){
            for($i=0;$i<count($info_arr);$i++){
                $data[$i]["jm_id"]=$info_arr[$i]["ww_id"];
                $data[$i]["ww_title"]=$info_arr[$i]["ww_title"];
                $data[$i]["ww_read"]=$info_arr[$i]["ww_read"];
                $data[$i]["ww_time"]=$info_arr[$i]["ww_time"];
                $data[$i]["ww_content"]=substr(strip_tags($info_arr[$i]["ww_content"]), 0, 300);
                $data[$i]["ww_content"]=str_replace("&nbsp;","",$data[$i]["ww_content"]);
                /**获取图片路径**/
...
//        $j->send(18,4,"",1,$msg_content,$platform);
    ...
    /** 创业点赞 */
    public function  Do_cy_good(){
        $ji_id=$this->getRequest()->get("ji_id");
        $jobinfo=new jobinfo();

        if($jobinfo->do_cy_good($ji_id)){
            $this->view->setAppStatus ( "1" );
            $this->view->setAppMsg ( "点赞成功" );
        }else{
            $this->view->setAppStatus ( "0" );
